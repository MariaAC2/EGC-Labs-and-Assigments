...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y -= deltaTime * 0.1f;
    }
    else if (window->KeyHold(GLFW_KEY_S)) {
        logicSpace.y += deltaTime * 0.1f;
    }
    else if (window->KeyHold(GLFW_KEY_A)) {
        logicSpace.x -= deltaTime * 0.1f;
    }
    else if (window->KeyHold(GLFW_KEY_D)) {
        logicSpace.x += deltaTime * 0.1f;
    }
    else if (window->KeyHold(GLFW_KEY_Z)) {
        float zoomRatio = deltaTime * 2;
        float width = logicSpace.width;
        float height = logicSpace.height;

        logicSpace.width -= zoomRatio;
        logicSpace.height -= zoomRatio;

        logicSpace.x -= (logicSpace.width - width) / 2;
        logicSpace.y -= (logicSpace.height - height) / 2;
    }
        ...
        logicSpace.x += (width - logicSpace.width) / 2;
        logicSpace.y += (height - logicSpace.height) / 2;
    }
