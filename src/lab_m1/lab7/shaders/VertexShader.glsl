...
void main()
{
    ...
    vec3 diffuse_light = light_color * material_kd * max (dot(world_normal,L), 0);
    ...
    {
        specular_light = light_color * material_ks * pow(max(dot(V, R), 0), material_shininess);
    }
	...
	vec3 light = ambient_light + attenuation_factor * (diffuse_light + specular_light);
